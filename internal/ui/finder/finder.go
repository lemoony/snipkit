...
// Required because of https://github.com/lemoony/snipkit/issues/157.
// In the long term, the finder primitive should be refactored to be
// based on bubbletea instead of tview.
...
func NewFinder() *Finder {
	search := &Finder{
		...
		inputLabelStyle:          tcell.StyleDefault.Background(tview.Styles.PrimitiveBackgroundColor).Foreground(tview.Styles.SecondaryTextColor),
		placeholderStyle:         tcell.StyleDefault.Background(tview.Styles.ContrastBackgroundColor).Foreground(tview.Styles.ContrastSecondaryTextColor),
		fieldStyle:               tcell.StyleDefault.Background(tview.Styles.ContrastBackgroundColor).Foreground(tview.Styles.PrimaryTextColor),
		itemLabelStyle:           tcell.StyleDefault.Background(tview.Styles.PrimaryTextColor).Foreground(tview.Styles.SecondaryTextColor),
		itemStyle:                tcell.StyleDefault.Background(tview.Styles.PrimitiveBackgroundColor).Foreground(tview.Styles.PrimaryTextColor),
		selectedItemStyle:        tcell.StyleDefault.Background(tview.Styles.PrimaryTextColor).Foreground(tview.Styles.PrimitiveBackgroundColor),
		selectedItemLabelStyle:   tcell.StyleDefault.Background(tview.Styles.PrimaryTextColor).Foreground(tview.Styles.TertiaryTextColor),
		counterStyle:             tcell.StyleDefault.Background(tview.Styles.PrimitiveBackgroundColor).Foreground(tview.Styles.SecondaryTextColor),
		highlightMatchStyle:      tcell.StyleDefault.Background(tview.Styles.PrimitiveBackgroundColor).Foreground(tview.Styles.TertiaryTextColor).Bold(true),
...
//
//nolint:funlen,gocyclo // not worth refactoring for now.
func (f *Finder) Draw(screen tcell.Screen) {
	...
	printWithStyleStripColors(
		...
		f.inputOffset(), currentY, 0, width, tview.AlignLeft, f.counterStyle, false, true,
	)
	drawnLines += 1
	currentY--
	...
	for index, matchItem := range f.matched {
		...
		// print label
		printWithStyleStripColors(
			screen, label, x, currentY, 0, width, tview.AlignLeft, labelStyle, false, false,
		)
		currentX += len(label)
		if selected {
			printWithStyleStripColors(
				screen, " ", currentX, currentY, 0, width, tview.AlignLeft, labelStyle, false, false,
			)
		}
		...
		printWithStyleStripColors(
			...
			itemTitle,
			currentX,
			currentY, 0, maxTitleWidth, tview.AlignLeft, itemStyle, false, true,
		)
...
func (f *Finder) drawInputField(screen tcell.Screen) {
	...
	// Draw inputLabel.
	_, drawnWidth, _, _ := printWithStyle(screen, f.inputLabel, x, currentY, 0, rightLimit-x, tview.AlignLeft, f.inputLabelStyle, false)
	x += drawnWidth
	...
	if showPlaceholder := text == ""; showPlaceholder {
		// Draw showPlaceholder text.
		printWithStyle(screen, tview.Escape(f.placeholder), x, currentY, 0, fieldWidth, tview.AlignLeft, f.placeholderStyle, true)
		f.offset = 0
	} else {
		// Draw entered text.
		if fieldWidth >= stringWidth(text) {
			// We have enough space for the full text.
			printWithStyle(screen, tview.Escape(text), x, currentY, 0, fieldWidth, tview.AlignLeft, inputStyle, true)
			f.offset = 0
			...
			printWithStyle(screen, tview.Escape(text[f.offset:]), x, currentY, 0, fieldWidth, tview.AlignLeft, inputStyle, true)
		}
	}
